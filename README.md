...
The download button links to the **signed + Apple-notarized** disk image
...
> The `.dmg` is hosted as a **GitHub Release asset** (not in this repo) because it
> exceeds GitHub's 100 MB per-file limit. The download buttons in `index.html` point
> at the release asset URL, e.g.
> `https://github.com/nomima-app/Nomima-landing/releases/download/v0.16.0-beta.1/Nomima_0.16.0-beta.1_aarch64.dmg`.
> To ship a new build: notarize + staple the new `.dmg`, upload it to a new GitHub
> Release (tag `vX.Y.Z`), then update the two `href`s in `index.html` and the
> `version`/`url` in `latest.json`.
